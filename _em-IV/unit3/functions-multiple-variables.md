---
layout: lesson
title: FUNCTIONS OF MULTIPLE VARIABLES
course: calculus-III
unit: unit3
lessonID: functions-multiple-variables
nextID: contour-plots
---

- Evaluate a function at a point in the plane
- dividing by zero is not ok
- some real life examples of functions of multiple variables
- Optional section on using a bit more formal notation.
### Introduction
In single variable calculus, we had functions of the form \\(y = f(x)\\), with a single variable depending on a single variable. In the last unit, we had functions of the form \\(\textbf{r}(t) = (x(t),y(t),z(t))\\), which was multiple variables \\((x,y,z\\)) depending on a single variable, \\(t\\). Here, we will be looking at functions which depend on multiple variables, for example \\(z = f(x,y)\\), or \\(u = f(x,y,z)\\). 

The limits on our abilities to graph functions stops at functions which take *two* independent variables and output a single dependent variable. Let's see some practical examples of functions of multiple variables:

### 


### Exercises

<ol>
<li> <div> Sketch all of the points such that \(y = 1\). </div>

<button onclick="myFunction('answer2')" class="answerButton">Show Answer</button>
<div  id="answer2" class="answer">
This is a plane that is parallel to the \(xz\) plane. 
</div> </li>
<li> <div> Sketch all of the points such that \(y = 1\). </div>

<button onclick="myFunction('answer2')" class="answerButton">Show Answer</button>
<div  id="answer2" class="answer">
This is a plane that is parallel to the \(xz\) plane. 
</div> </li>
</ol>
